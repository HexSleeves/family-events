...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('userDropdown', () => ({
    ...
    get items() {
      return this.$refs.dropdown ? Array.from(this.$refs.dropdown.querySelectorAll('[role="menuitem"]')) : [];
    },
    focusItem(idx) {
  ...
  Alpine.data('toastContainer', () => ({
    ...
    iconFor(variant) {
      return { success: 'check-circle', error: 'x-circle', warning: 'alert-triangle', info: 'info' }[variant] || 'info';
    },
    bgFor(variant) {
      return {
  ...
  Alpine.data('passwordStrength', () => ({
    ...
    get strengthColor() {
      return ['bg-[var(--color-edge)]', 'bg-[var(--color-danger)]', 'bg-[var(--color-warning)]', 'bg-[var(--color-info)]', 'bg-[var(--color-success)]'][this.strength];
    }
  }));
...
document.body.addEventListener('htmx:configRequest', (evt) => {
  ...
  if (target && target.id === 'bulk-unattend-form') {
    const ids = Array.from(target.querySelectorAll('.attended-select:checked')).map(el => el.value);
    if (!ids.length) {
      evt.preventDefault();
